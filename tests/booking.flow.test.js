...
describe('Booking CRUD flow', () => {
    ...
    it('Create new booking', async() => {
        const { response, duration } = await measure(() => bookingApi.createBooking(newBooking()));
    ...
    it('Read booking', async() => {
        const { response, duration } = await measure(() => bookingApi.getBookingById(id));
        assertions.assertResponse({response, duration, expectedStatus: 200});
    ...
    it('Update booking', async() => {
        ...
        const { response, duration } = await measure(() => bookingApi.updateBooking(id, updated, { Authorization: basicAuthorization }));
        assertions.assertResponse({response, duration, expectedStatus: 200});
        assertions.assertSchema(bookingSchemas.bookingData, response.data);
    });

    it('Delete booking', async() => {
        const { response } = await measure(() => bookingApi.deleteBooking(id, { Authorization: basicAuthorization }));
